...
const Join = ({t}) => {
  ...
  return (
    <section className='relative  flex flex-col justify-center items-center gap-12 text-center min-h-[100vh]'>
        ...
        <button ref={btnRef as React.RefObject<HTMLButtonElement>} onClick={showAlert} className={`slide-in-elliptic-top-fwd-pre ${btnAnimation} bg-yellow-800 md:text-lg btn-diagonal-swipe max-md:px-4 max-md:py-2 text-white`}>{t['btn']}</button>
    </section>
  )
}
